...
from snapshot.review import paths, splits
...
class TrainState:
    ...
    # ABSOLUTE, VIA paths. This was Path("vision/plate_model.pt") - relative, so it only
    ...
    def save_checkpoint(self, name: str | None = None, folder: str = "") -> dict:
        ...
        plate_model.pt is overwritten by every run; these are not. the name and the folder are
        picked in the page, both defaulting to what was used before
        """
        ...
        (checkpoints / f"{stem}.classes.json").write_text(
            json.dumps(
                {
    ...
    def window_floor(self) -> dict:
        ...
        return {"floor": floor, "box": [width, height], "default": max(CROP, floor)}
    ...
    def start(
        ...
        self._worker = threading.Thread(
            target=self._run, args=(epochs, batch, window, rate, run_seed), daemon=True
        )
        self._worker.start()
        return {"ok": True}
    ...
    def _run(self, epochs: int, batch: int, window: int, rate: float, seed: int) -> None:
        try:
    ...
    def _holdout_score(self, model) -> dict | None:
        ...
        from smolsmort.detect.model import PEAK_MIN_SCORE, decode_peaks
        ...
        for example in self.holdout:
            ...
            peaks = decode_peaks(hot, limit=HOLDOUT_PEAK_LIMIT)
